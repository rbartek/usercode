/**
 ...
 \verbatim
 mkdir [directoryName]
 TriggerStudyMuon <inputfile> [outputfilename] [directoryName]
 \endverbatim
...
int main(int argc, char** argv) {
    ...
    std::string ofilename("trigMu.root");
	...
	double etas[] = {0.,0.5,1.0,1.5,2.1,2.4};
	...
	TH1F *hPtPass = new TH1F	("hPtPass",  "hPt Pass Muon",					9, pts);
	TH1F *hEtaPass = new TH1F	("hEtaPass",  "hEta Pass Muon",	5, etas);
	TH1F *hPtAll = new TH1F	("hPtAll",  "hPt All Muon",					9, pts);
	TH1F *hEtaAll = new TH1F	("hEtaAll",  "hEta All Muon",	5, etas);
	hPtPass->Sumw2();
	hEtaPass->Sumw2();
	hPtAll->Sumw2();
	hEtaAll->Sumw2();
	
	TH1F *hPtTurnOnCurve0_05;
	TH1F *hPtPass0_05 = new TH1F	("hPtPass0_05",  "hPt Pass Muon 0 < eta < 0.5",					9, pts);
	TH1F *hPtAll0_05 = new TH1F	("hPtAll0_05",  "hPt All Muon 0 < eta < 0.5",					9, pts);
	TH1F *hPtTurnOnCurve05_10;
	TH1F *hPtPass05_10 = new TH1F	("hPtPass05_10",  "hPt Pass Muon 0.5 < eta < 1.0",					9, pts);
	TH1F *hPtAll05_10 = new TH1F	("hPtAll05_10",  "hPt All Muon  0.5 < eta < 1.0",					9, pts);
	TH1F *hPtTurnOnCurve10_15;
	TH1F *hPtPass10_15 = new TH1F	("hPtPass10_15",  "hPt Pass Muon 1.0 < eta < 1.5",					9, pts);
	TH1F *hPtAll10_15 = new TH1F	("hPtAll10_15",  "hPt All Muon 1.0 < eta < 1.5",					9, pts);
	TH1F *hPtTurnOnCurve15_21;
	TH1F *hPtPass15_21 = new TH1F	("hPtPass15_21",  "hPt Pass Muon 1.5 < eta < 2.1",					9, pts);
	TH1F *hPtAll15_21 = new TH1F	("hPtAll15_21",  "hPt All Muon 1.5 < eta < 2.1",					9, pts);
	TH1F *hPtTurnOnCurve21_24;
	TH1F *hPtPass21_24 = new TH1F	("hPtPass21_24",  "hPt Pass Muon 2.1 < eta < 2.4",					9, pts);
	TH1F *hPtAll21_24 = new TH1F	("hPtAll21_24",  "hPt All Muon 2.1 < eta < 2.4",					9, pts);
		...
		if (((isM50sample && sample.genZpt < 100) || !isM50sample) && ((HFfile && sample.eventFlav == 5)||!HFfile) && ((LFfile && sample.eventFlav!=5)||!LFfile)){
			...
			if (sample.Vtype == 5 && sample.EVENT_json == 1 ){
				...
				if ( jetPt[0] > 20 && jetPt[1] > 20 && 
					...
					if(delRemu>0.4){
						if(sample.vLepton_pt[0]>30){
						//&&indexedJetPt[0].second>30&&indexedJetPt[1].second>30){
						//	"HLT_IsoMu17_v.*" , #0
...
//						if ((sample.triggerFlags[17]||sample.triggerFlags[18]||sample.triggerFlags[19]||sample.triggerFlags[26]||sample.triggerFlags[27])){
						//	if (sample.triggerFlags[18]){
						N_EfakeCuts = N_EfakeCuts + 1*PUweight2011;
						hPtAll->Fill(sample.vLepton_pt[1]);
						hEtaAll->Fill(fabs(sample.vLepton_eta[1]));
						if (sample.triggerFlags[40]){
							hPtPass->Fill(sample.vLepton_pt[1]);
							hEtaPass->Fill(fabs(sample.vLepton_eta[1]));
						}
							for(int i = 0; i<5; i++){
								if(sample.vLepton_eta[1] > etas[i] && sample.vLepton_eta[1] < etas[i+1]){
									switch (i){
										case 0:
											hPtAll0_05->Fill(sample.vLepton_pt[1]);
											if (sample.triggerFlags[40]) hPtPass0_05->Fill(sample.vLepton_pt[1]);
											break;
										case 1:
											hPtAll05_10->Fill(sample.vLepton_pt[1]);
											if (sample.triggerFlags[40]) hPtPass05_10->Fill(sample.vLepton_pt[1]);
											break;
										case 2:
											hPtAll10_15->Fill(sample.vLepton_pt[1]);
											if (sample.triggerFlags[40]) hPtPass10_15->Fill(sample.vLepton_pt[1]);
											break;
										case 3:
											hPtAll15_21->Fill(sample.vLepton_pt[1]);
											if (sample.triggerFlags[40]) hPtPass15_21->Fill(sample.vLepton_pt[1]);
											break;
										case 4:
											hPtAll21_24->Fill(sample.vLepton_pt[1]);
											if (sample.triggerFlags[40]) hPtPass21_24->Fill(sample.vLepton_pt[1]);
											break;											
										default:
	...
	ofstream myfile;
	myfile.open(TString::Format("%sMuonTrigEff.txt",directory.c_str()).Data());	
	...
	myfile<<TString::Format("pass eta bins \t %0.0f \t %0.0f  \t %0.0f ",hEtaPass->GetBinContent(1),hEtaPass->GetBinContent(2),hEtaPass->GetBinContent(3)).Data()<<endl;
	myfile<<TString::Format("all eta bins \t %0.0f \t %0.0f  \t %0.0f ",hEtaAll->GetBinContent(1),hEtaAll->GetBinContent(2),hEtaAll->GetBinContent(3)).Data()<<endl;
	myfile<<TString::Format("eff eta bins \t %0.5f \t %0.5f \t %0.5f",hEtaPass->GetBinContent(1)/hEtaAll->GetBinContent(1),
							hEtaPass->GetBinContent(2)/hEtaAll->GetBinContent(2),hEtaPass->GetBinContent(3)/hEtaAll->GetBinContent(3)).Data()<<endl;
	
	//hPtTurnOnCurve = (TH1F*)hPtPass->Clone("PtTurnOnCurve");
	//hEtaTurnOnCurve = (TH1F*)hEtaPass->Clone("EtaTurnOnCurve");
	hPtTurnOnCurve = new TH1F	("hPtTurnOnCurve",  "hPtTurnOnCurve Muon",		9, pts);
	hEtaTurnOnCurve = new TH1F	("hEtaTurnOnCurve",  "hEtaTurnOnCurve Muon",	5, etas);
	...
	myfile<<TString::Format("error eta bins \t %0.5f \t %0.5f  \t %0.5f ",hEtaTurnOnCurve->GetBinError(1),hEtaTurnOnCurve->GetBinError(2),hEtaTurnOnCurve->GetBinError(3)).Data()<<endl;
	
	myfile<<endl;
	myfile.close();
	
	hPtTurnOnCurve0_05 = new TH1F	("hPtTurnOnCurve0_05",  "hPtTurnOnCurve Muon 0 < eta < 0.5",		9, pts);
	hPtTurnOnCurve0_05->Sumw2();
	hPtTurnOnCurve0_05->Divide(hPtPass0_05,hPtAll0_05,1,1,"B");
	hPtTurnOnCurve05_10 = new TH1F	("hPtTurnOnCurve05_10",  "hPtTurnOnCurve Muon 0.5 < eta < 1.0",		9, pts);
	hPtTurnOnCurve05_10->Sumw2();
	hPtTurnOnCurve05_10->Divide(hPtPass05_10,hPtAll05_10,1,1,"B");
	hPtTurnOnCurve10_15 = new TH1F	("hPtTurnOnCurve10_15",  "hPtTurnOnCurve Muon 1.0 < eta < 1.5",		9, pts);
	hPtTurnOnCurve10_15->Sumw2();
	hPtTurnOnCurve10_15->Divide(hPtPass10_15,hPtAll10_15,1,1,"B");
	hPtTurnOnCurve15_21 = new TH1F	("hPtTurnOnCurve15_21",  "hPtTurnOnCurve Muon 1.5 < eta < 2.1",		9, pts);
	hPtTurnOnCurve15_21->Sumw2();
	hPtTurnOnCurve15_21->Divide(hPtPass15_21,hPtAll15_21,1,1,"B");
	hPtTurnOnCurve21_24 = new TH1F	("hPtTurnOnCurve21_24",  "hPtTurnOnCurve Muon 2.1 < eta < 2.4",		9, pts);
	hPtTurnOnCurve21_24->Sumw2();
	hPtTurnOnCurve21_24->Divide(hPtPass21_24,hPtAll21_24,1,1,"B");
			...
			for(int i = 0; i<5; i++){
					for(int j = 0; j<9; j++){
							...
							switch (i){
								case 0:
									scale = hPtTurnOnCurve0_05->GetBinContent(j);
									error = hPtTurnOnCurve0_05->GetBinError(j);
									break;
								case 1:
									scale = hPtTurnOnCurve05_10->GetBinContent(j);
									error = hPtTurnOnCurve05_10->GetBinError(j);
									break;
								case 2:
									scale = hPtTurnOnCurve10_15->GetBinContent(j);
									error = hPtTurnOnCurve10_15->GetBinError(j);
									break;
								case 3:
									scale = hPtTurnOnCurve15_21->GetBinContent(j);
									error = hPtTurnOnCurve15_21->GetBinError(j);
									break;
								case 4:
									scale = hPtTurnOnCurve21_24->GetBinContent(j);
									error = hPtTurnOnCurve21_24->GetBinError(j);
									break;
								default:
									cout << "ERROR ERROR ERROR BINS ARE WRONG" << endl;
							}//end switch
