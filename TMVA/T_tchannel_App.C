...
void T_tchannel_App( TString myMethodList = "" ) 
{   
#ifdef __CINT__
   ...
   std::cout << std::endl;
   std::cout << "==> Start T_tchannel_App" << std::endl;
...
*/
   ...
   //   
   TFile *input(0);
   TString fname = "/home/hep/wilken/Weights/CMSSW_4_2_8_patch3/src/UserCode/wilken/CSVShapeCorr/T_tchannel.root"; 
	double lumi = 100.0;
	Double_t  T_tchannel_weight =  lumi/(lumiTt/2.0) ; //T_TuneZ2_t-channel_7TeV-powheg-tauola
   ...
   //   var2 = userVar1 - userVar2;
      ...
      if (Use["TMlpANN"      ])   histNnT    ->Fill( reader->EvaluateMVA( "TMlpANN method"       ) );
	   if (Use["BDT"          ]) {
		   ...
		   histMattBdt    ->Fill( BDTvalue,T_tchannel_weight );
		   histTMVABdt    ->Fill( BDTvalue,T_tchannel_weight );
	   }
...
hMjj_OpenSelection->Fill(Hmass,T_tchannel_weight);
hMmumu_OpenSelection->Fill(Zmass,T_tchannel_weight);
hPtjj_OpenSelection->Fill(Hpt,T_tchannel_weight);
hPtmumu_OpenSelection->Fill(Zpt,T_tchannel_weight);
hCSV0_OpenSelection->Fill(CSV0,T_tchannel_weight);
hCSV1_OpenSelection->Fill(CSV1,T_tchannel_weight);
hdphiVH_OpenSelection->Fill(DeltaPhiHV,T_tchannel_weight);
hdetaJJ_OpenSelection->Fill(DetaJJ,T_tchannel_weight);
hUnweightedEta_OpenSelection->Fill(UnweightedEta,T_tchannel_weight);
hPtmu0_OpenSelection->Fill(mu0pt,T_tchannel_weight);
	   hHt_OpenSelection->Fill(Ht,T_tchannel_weight);
	   hCircularity_OpenSelection->Fill(EvntShpCircularity,T_tchannel_weight);
	   hCHFb0_OpenSelection->Fill(jetCHF0, T_tchannel_weight);
	   hCHFb1_OpenSelection->Fill(jetCHF1, T_tchannel_weight);
	   hPtbalZH_OpenSelection->Fill(PtbalZH, T_tchannel_weight);
	   hPtmu1_OpenSelection->Fill(mu1pt, T_tchannel_weight);
	   hPFRelIsomu0_OpenSelection->Fill(muonPFiso0, T_tchannel_weight);
	   hPFRelIsomu1_OpenSelection->Fill(muonPFiso1, T_tchannel_weight);
	   hNjets_OpenSelection->Fill(nJets, T_tchannel_weight);
	   hRMSeta_OpenSelection->Fill(RMS_eta, T_tchannel_weight);
	   hStaDeveta_OpenSelection->Fill(EtaStandDev, T_tchannel_weight);
	   hdphiJJ_vect_OpenSelection->Fill(DphiJJ, T_tchannel_weight);
	   hCentrality_OpenSelection->Fill(Centrality, T_tchannel_weight);
	   hEventPt_OpenSelection->Fill(EventPt, T_tchannel_weight);
	   hAngle_OpenSelection->Fill(Angle, T_tchannel_weight);
	   hSphericity_OpenSelection->Fill(EvntShpSphericity, T_tchannel_weight);
	   hAplanarity_OpenSelection->Fill(EvntShpAplanarity, T_tchannel_weight);
	   hIsotropy_OpenSelection->Fill(EvntShpIsotropy, T_tchannel_weight);
	   hDphiDetajj_OpenSelection->Fill(DphiJJ, DetaJJ, T_tchannel_weight);
   ...
   TFile *target  = new TFile( "TMVApp_T_tchannel.root","RECREATE" );
...
treeWithBDT->Delete();
   ...
   std::cout << "==> T_tchannel_App is done!" << endl << std::endl;
