...
{
 public: 
   BTagShapeNew(const char * file) : m_file(file)
   {
...
//    float integral =  h->Integral(-1,lastbin);
	int cutAndSFsize = cutsAndSF.size();
    for(int i =0;i<cutAndSFsize; i++)
    {
     ...
     if (debug) std::cout << std::endl<<    " Scale Factor : " << sf << std::endl;
//      float target=originalIntegral/sf;
      float target=originalIntegral*sf;
       if (debug) std::cout << " Target " << target << " orig " << originalIntegral << std::endl;
      for(int j=lastbin; j> -1; j--)
      {
        if(h->Integral(j,lastbin)>= target)
	      ...
	      if (debug) std::cout << "Found at " << j << " was " << h->FindBin(oldCut) <<  std::endl;
	      if (debug) std::cout << h->GetBinLowEdge(j) << " was " << originalLowEdge << " cut: " << oldCut <<  std::endl;
             break;
          }
      }
...
/*  TGraph * makeGraph(TH1F * h,const std::vector<std::pair<float, float> > & cutsAndSF)
  {
	  ...
	  int eqSize = eq.size();
      for(int i  = 1 ; i < MAXPOINTS-2 && i <= eqSize; i++)
      {
...
// new
    ...
    cutsAndSFB.push_back(std::pair<float, float>(0.679,0.97+uncert*0.03));
    cutsAndSFB.push_back(std::pair<float, float>(0.244,1.00+uncert*0.03));
