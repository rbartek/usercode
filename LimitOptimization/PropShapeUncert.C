...
void PropShapeUncert(){
	...
	string directory = "/home/hep/wilken/limit/CMSSW_4_2_8/src/BinUncertainty/";
	TString plot = directory+"somereallylongnameherehopethisarrayisbiggenough"+suffixps;	
	
	float minLimit = 999999.99, corrCut = -99.99, significance, SoB;
	...
	float BestCutValue[] = {45, //0	Higgs mass min
		150, //1	Higgs mass max
		0.0, //2	CSV0
		10, //3		Z mass min
		70, //4		Z mass max
	1.00}; //5 Delta Phi (Z,MET)
	...
	string VarNames[] = {"HmassMin","HmassMax","CSV0","ZmassMin","ZmassMax","DeltaPhiZMET"};
	
	float cut = -99.99;
	double BDTvalue = -99.99;
	
	//	for (int Var_iter = 0; Var_iter<6;Var_iter++){
	int Var_iter = 2;
	...
	float BinMax = -0.2;
	float BinMin = -1.0;
	float HistoRange = abs(BinMin-BinMax);
	float BinSize = HistoRange/(float)NumberOfBins;
	cout << "Bin Size " <<BinSize << endl;
	for (int ibin =0; ibin<(NumberOfBins+1); ibin++){
		int nbins = 1;
		float bl=-1.0, bh=-0.2;
		bl = ibin*BinSize+BinMin;
		bh = ibin*BinSize+BinMin+BinSize;
		cout << "Bin Min " <<bl<< " Bin Max " <<bh << endl;
		...
		{
			string filename;
			filename = TString::Format("HistoForShapeLimit%0.0f.root",(double)ibin).Data();
			...
			for(unsigned int isample = 0; isample<count.size(); isample++){
				...
				for (Long64_t ievt=0; ievt<sample[isample]->GetEntries();ievt++) {
					...
					if((lep0pt > 20 || lep1pt >20) && (lep0pt > 10 && lep1pt >10)&&delRemu > 0.4){
						...
						switch (Var_iter){
							case 0:
								if (Hmass < BestCutValue[1] && CSV0 > BestCutValue[2] && Emumass > BestCutValue[3] && Emumass < BestCutValue[4] && fabs(DphiZMET)  < BestCutValue[5]){
									if(Hmass>cut) {
								...
								break;																								
							case 1:
								if (Hmass > BestCutValue[0] && CSV0 > BestCutValue[2] && Emumass > BestCutValue[3] && Emumass < BestCutValue[4] && fabs(DphiZMET)  < BestCutValue[5]){
									if(Hmass<cut) {
										EventsBDT[isample]->Fill(BDTvalue,lumiScale[isample]*PUweight2011);
										count[isample]++;
									}}
								break;																
							case 2:
								if (Hmass > BestCutValue[0] && Hmass < BestCutValue[1] && Emumass > BestCutValue[3] && Emumass < BestCutValue[4] && fabs(DphiZMET)  < BestCutValue[5]){
									if(CSV0 >cut) {
										EventsBDT[isample]->Fill(BDTvalue,lumiScale[isample]*PUweight2011);
										count[isample]++;
									}}
								break;								
							case 3:
								if (Hmass > BestCutValue[0] && Hmass < BestCutValue[1] && CSV0 > BestCutValue[2] && Emumass < BestCutValue[4] && fabs(DphiZMET)  < BestCutValue[5]){
									if(Emumass >cut) {
										EventsBDT[isample]->Fill(BDTvalue,lumiScale[isample]*PUweight2011);
										count[isample]++;
									}}
								break;								
							case 4:
								if (Hmass > BestCutValue[0] && Hmass < BestCutValue[1] && CSV0 > BestCutValue[2] && Emumass > BestCutValue[3] && fabs(DphiZMET)  < BestCutValue[5]){
									if(Emumass <cut) {
										EventsBDT[isample]->Fill(BDTvalue,lumiScale[isample]*PUweight2011);
										count[isample]++;
									}}
								break;
							case 5:
								if (Hmass > BestCutValue[0] && Hmass < BestCutValue[1] && CSV0 > BestCutValue[2] && Emumass > BestCutValue[3] && Emumass < BestCutValue[4]){
			...
			cout << "Bin " <<ibin << " Number of Events Signal: " <<count[0] << " TTJets " << count[1]<< " DY M50 LF " << count[2]<< endl;
			...
			for(unsigned int j =0; j<sample.size(); j++){
				...
				double sigma =-99.99;
				binContent = EventsBDT[j]->GetBinContent(1);
				cout << "BinContent of " <<SampleName[j] << " is " << binContent << endl;
				sigma = EventsBDT[j]->GetBinError(1);
				h_stat_up[j]->SetBinContent(1,binContent+sigma);
				//if((binContent-sigma) < 0) sigma  = 0;
				h_stat_down[j]->SetBinContent(1,binContent-sigma);
			...
			for(unsigned int k =1; k<sample.size(); k++){
				...
				if ((EventsBDT[k]->Integral() < 0.001)||(h_stat_down[k]->Integral() < 0.0000001)){
					EventsBDT[k]->Reset("ICES");
					h_stat_up[k]->Reset("ICES");
					h_stat_down[k]->Reset("ICES");
				}
			}
			
			makeDataCard(EventsBDT[0]->Integral(), EventsBDT[2]->Integral(), EventsBDT[4]->Integral(), EventsBDT[1]->Integral(), EventsBDT[6]->Integral(), EventsBDT[10]->Integral(), (float)ibin, directory, filename);
			...
			float ZH = EventsBDT[0]->Integral();
			float Bkg = EventsBDT[2]->Integral()+EventsBDT[4]->Integral()+EventsBDT[1]->Integral()+EventsBDT[6]->Integral()+EventsBDT[10]->Integral();
			...
			myLegend.Draw();		
			//plot = directory+"DphiZMET"+ibin.c_str()+suffixps;
			plot = TString::Format("%sBDT%0.0f.gif",directory.c_str(),(double)ibin).Data();
			c1->Print(plot);
			c1->Clear();	
			...
			string combiner = TString::Format("combine -M ProfileLikelihood %sShapeDataCard%0.0fbin.txt -m 115 -t 1000 | tee %soutput%0.0fbin.log",directory.c_str(),(double)ibin,directory.c_str(),(double)ibin).Data();
			cout << combiner << endl;
			gSystem->Exec(combiner.c_str());
			string GetMedianLimit  = TString::Format("grep 'median expected' %soutput%0.0fbin.log  | gawk '{print $6}' > tmpMedianLimit.out",directory.c_str(),(double)ibin).Data();
			cout << GetMedianLimit << endl;
			...
			cout << "limit is " <<medianExpLim << " at " << ibin  << " S/B " << ZH/Bkg << endl;
...
void makeDataCard(float VHrate, float ZjLFrate, float ZjHFrate, float TTrate, float sToprate, float VVrate, float Thiscut, string dir, string shapefile){
	ofstream myfile (TString::Format("%s/ShapeDataCard%0.0fbin.txt",dir.c_str(),Thiscut).Data());
