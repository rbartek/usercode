...
void MinLimFinder(){
	...
	string directory = "/home/hep/wilken/limit/CMSSW_4_2_8/src/SecondLoop/";
	TString plot = directory+"somereallylongnameherehopethisarrayisbiggenough"+suffixps;	
	
	float minLimit = 9999.99, corrCut = -99.99, significance, SoB, GlobalMin = 9999.99, GlobalMinCut=-99.99;
	...
	float cutmin[] = {50, 100, 0, 0, 50, 0.2};
	float cutmax[] = {100, 300, 1, 20, 200, 1.2};
//	float csvsteps[] = {0, 0.244, 0.5, 0.679, 0.898};
	float csvsteps[] = {0, 0.1, 0.244, 0.3, 0.4};
	
	float range = -99.99; 
	float cut = -99.99;
	double BDTvalue = -99.99;
	
	//	for (int Var_iter = 5; Var_iter>-1;Var_iter--){
 ...
 for (int icut = 0; icut<steps; icut++){
			if(Var_iter!=2) cut = icut*stepsize+cutmin[Var_iter];
			...
			Punzi->SetPoint(icut,cut,significance*220000);
			
			if (medianExpLim<minLimit) {
				minLimit = medianExpLim;
				corrCut = cut;
			}
			if (medianExpLim<GlobalMin) {
				GlobalMin = medianExpLim;
				GlobalMinCut = cut;
			}
